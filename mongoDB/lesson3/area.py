...
CITIES = '../cities.csv' # if change to cities_udacity.csv, the test will be correct
...
def fix_area(area):
    ...
    if area == "NULL" or area == "":
        ...
        separator_pos = findOccurences(area, "|")
        current_pos = 1 # skip "{"
        i = 0
        temp = []
     
        while i < len(separator_pos):
            temp.append(area[current_pos:separator_pos[i]])
            current_pos +=  separator_pos[i]
            i += 1
        temp.append(area[current_pos:len(area)-1]) # add the last string, skip "}"
        area = float(max(temp, key=len)) # choose area with more significant digits
    else:
        area = float(area)
    return area
